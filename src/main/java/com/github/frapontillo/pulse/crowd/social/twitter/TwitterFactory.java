package com.github.frapontillo.pulse.crowd.social.twitter;
...
public class TwitterFactory {
    ...
    /**
     * Stops the current thread for the amount of time needed to recover from a "rate limit
     * exceeded" error on the Twitter API side.
     *
     * @param exception The original rate limit exception to retrieve the amount of time to wait
     *                  from.
     * @param logger    The logger to use to print messages.
     *
     * @return {@code true} when the rate limit should be expired, {@code false} if the exception
     * was not rate limit related.
    ...
    public static boolean waitForTwitterTimeout(TwitterException exception, Logger logger)
        ...
        if (exception.getCause() != null &&
                exception.getCause().getClass() == FileNotFoundException.class) {
            return true;
        }

        // if the error code stands for "Over capacity", retry
        if (exception.getErrorCode() == 130) {
            waitWithMessage(3, "Twitter is over capacity right now, waiting for {} seconds...",
                    logger);
            return true;
        }
        ...
        if (exception.getCause() instanceof UnknownHostException) {
            waitWithMessage(3,
                    "Couldn't find Twitter host, probably because of some network error, waiting " +
                            "for {} seconds...", logger);
            return true;
        }
        ...
        if (exception.getRateLimitStatus() != null) {
            ...
            if (remaining <= 0) {
                ...
                if (secondsToWait <= 0) {
                    logger.warn("Got a negative (" + secondsToWait + ") seconds to wait.",
                            exception);
                    secondsToWait = 30;
                }
                waitWithMessage(secondsToWait,
                        "Encountered Twitter rate limit, waiting for {} seconds...", logger);
    ...
    private static void waitWithMessage(int secondsToWait, String message, Logger logger)
            throws InterruptedException {
